...
- **Phase 2 (detailed below):** a strategy backtesting engine, chart pattern detection beyond candlesticks (Head & Shoulders, ABCD, Dow trend structure), a technical rating + Analyzer page, the confidence-under-pressure drill mode and expanded topic curation in Learn, intraday scanning and volume-based ranking in Signals, plus — further out still — a long-term personal trading-pattern profile built from many mentor sessions over time.
...
## Phase 2 — Chart Pattern Detection Beyond Candlesticks

### The problem this solves

The "fact layer" that Analyse, Learn, and Signals all lean on today only recognizes **candlestick** patterns — Engulfing, Doji, Morning/Evening Star, and the rest of the 27-pattern library, each completing over one to three candles. It has no concept of **multi-swing chart patterns** — Head and Shoulders, ABCD, or the basic Dow-theory trend structure (sequences of higher-highs/higher-lows vs. lower-highs/lower-lows) that most discretionary trend calls are actually built on. If a trader's stated strategy in Analyse is "I trade breakouts off a Head and Shoulders neckline," today's fact layer has nothing to check that read against — the mentor can't fact-ground a conversation about a pattern it can't see.

This also caps Signals' Layer 1 pattern match (see Signals section above) at candlestick setups only, and would be a direct input to a Technical Rating + Analyzer page (below) if that gets built, since setup-quality scoring needs to know what structural pattern, if any, is forming.

### Solution shape

Two candidate approaches, not mutually exclusive: our own pivot-based geometric detection (identify swing highs/lows, then pattern-match sequences of pivots against known shapes — mirrors how `patterns.js` detects candlesticks today, but geometric/ratio-based instead of fixed-shape, so it needs real backtesting before it's trusted), or a Pine Script study added to the TradingView widget (faster to ship, but detection logic lives outside our own codebase/DB, so it can't feed the fact layer, the 3-eye check, or Signals directly). Dow trend structure is the natural starting point — it's the simplest (pure pivot sequencing, no shape-matching) and it's also the input primitive Head & Shoulders and ABCD detection both need anyway.

Full design and open questions live in `docs/feature-backlog/chart-pattern-detection.md`.

---

## Phase 2 — Technical Rating + Analyzer Page

### The problem this solves

Every page in this redesign — Analyse, Journal, Insights, Signals — is triggered by a stock the trader already has in mind, whether from their own idea or a Signals result. None of them answer the more exploratory question: *"I don't have a specific setup yet — show me what's technically interesting about this stock, or across my universe, right now."* Chartmill's rating feature answers this with two scores: an overall technical-health score (trend, momentum, relative strength, moving-average structure) and a separate setup-quality score specifically for breakout readiness (base tightness, volume dry-up, distance from trigger level). Their Analyzer page then uses that scoring to surface candidate trade setups and a written analysis for a stock the trader is just considering, no chart upload required.

### How this fits Wick's model, not just Chartmill's

Same separation of concerns as the rest of this PRD: **the scores are rule-based and deterministic** (composite of trend, momentum, relative strength, volatility, base/volume structure — the same "facts, not opinions" layer Analyse's questioning is grounded in), and **Claude narrates on top** of the computed numbers rather than inventing them — consistent with how Analyse and Journal already combine `technicalindicators` detection with Claude's judgment, never asking Claude to be the source of a number the trader might rely on. This keeps the ratings reproducible and auditable rather than a black-box "AI score."

Where this sits relative to existing pages: closer to a screener/research tool than a per-trade evaluation. Likely complements Signals — Signals says "here's what matches your personal edge," this would say "here's a graded read on any stock you ask about, plus a narrative" — and depends on chart pattern detection (above) for setup identification beyond candlesticks, since a breakout setup-quality score is meaningless without first knowing what structure is actually forming.

Full design, the health/setup-quality score composition, and open questions (relative-strength benchmark data, page placement, journaling integration) live in `docs/feature-backlog/technical-rating-and-analyzer.md`.
